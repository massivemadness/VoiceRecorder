...
JNIEXPORT jint
Java_com_test_voicerecorder_codec_Opus_startRecord(JNIEnv *env, jclass class, jstring path) {
...
JNIEXPORT jint
Java_com_test_voicerecorder_codec_Opus_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame((uint8_t *) frameBytes, (size_t) len);
}

JNIEXPORT void Java_com_test_voicerecorder_codec_Opus_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}
...
int initPlayer(const char *path) {
    ...
    logi(TAG_VOICE, "initPlayer seekable:%d totalPcmDuration:%ld", _isSeekable ? 1 : 0,
         _totalPcmDuration);
...
JNIEXPORT jlong Java_com_test_voicerecorder_codec_Opus_getTotalPcmDuration(JNIEnv *env, jclass class) {
    return _totalPcmDuration;
}

JNIEXPORT void Java_com_test_voicerecorder_codec_Opus_readOpusFile(JNIEnv *env, jclass class, jobject buffer,
...
JNIEXPORT jint Java_com_test_voicerecorder_codec_Opus_seekOpusFile(JNIEnv *env, jclass class, jfloat position) {
    return seekPlayer(position);
}

JNIEXPORT jint Java_com_test_voicerecorder_codec_Opus_openOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT jint Java_com_test_voicerecorder_codec_Opus_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
Java_com_test_voicerecorder_codec_Opus_getWaveform(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
    if (pathStr == NULL) {
    ...
    if (opusFile == NULL || error != OPUS_OK) {
        ...
        (*env)->
                ReleaseStringUTFChars(env, path, pathStr
        );
        return
                result;
    }
...
//free(sampleBuffer);
    ...
    (*env)->
            ReleaseByteArrayElements(env, result, bytes, JNI_COMMIT
    );
    free(samples);
...
JNIEXPORT jbyteArray
Java_com_test_voicerecorder_codec_Opus_getWaveform2(JNIEnv
                                                 *env,
                                                         jclass class, jshortArray
                                                 array,
                                                         jint length
) {
    ...
    (*env)->
            ReleaseShortArrayElements(env, array, sampleBuffer,
                                      0);
    ...
    if (result) {
        ...
        (*env)->
                SetByteArrayRegion(env, result,
                                   0, bitstreamLength, (jbyte *) bytes);
    }
    free(samples);
